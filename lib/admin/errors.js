      ...
      try {
        ...
        if (!error && errorLogs) {
          ...
          errors = errorLogs.map(err => ({
            id: err.id,
            type: err.error_type,
            message: err.message || err.error_message, // Support both field names
            severity: err.metadata?.severity || 'medium',
            timestamp: err.created_at,
            resolved: err.resolved,
            screen: err.screen || err.screen_name, // Support both field names
            device: err.device_model || err.device_type || `${err.platform} ${err.platform_version}`, // Support both field names
            appVersion: err.app_version,
            stack: err.stack || err.error_stack, // Support both field names
