  ...
  try {
    ...
    if (supabase) {
      ...
      if (!subsError && subsData) {
        ...
        const revenueData = [];
        // Show last 12 months, starting from 12 months ago to current month
        for (let i = 11; i >= 0; i--) {
          // Calculate month: i=11 means 12 months ago, i=0 means current month
          let targetMonth = currentMonth - i;
          let targetYear = currentYear;
          
          // Handle year rollover (when going back past January)
          if (targetMonth < 0) {
            targetMonth += 12;
            targetYear -= 1;
          }
