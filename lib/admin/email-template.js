...
function generateEmailTemplate(options = {}) {
  ...
  if (personalizedMessage.includes('[App Store]') || personalizedMessage.includes('[Download on App Store]')) {
    ...
    // Email clients often block external images, so we use a styled button with SVG icon
    htmlMessage += `<p style="margin: 30px 0 20px 0; text-align: center;">
      <a href="${finalAppStoreUrl}" target="_blank" rel="noopener noreferrer" style="display: inline-flex; align-items: center; justify-content: center; text-decoration: none; padding: 12px 24px; background: #000000; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.2); color: #ffffff; font-size: 15px; font-weight: 600; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; letter-spacing: 0.3px; gap: 8px;">
        <span style="display: inline-block; width: 18px; height: 18px; vertical-align: middle;">
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg" style="display: block;">
            <path d="M13.5 1.5C13.5 0.67 12.83 0 12 0H6C5.17 0 4.5 0.67 4.5 1.5V16.5C4.5 17.33 5.17 18 6 18H12C12.83 18 13.5 17.33 13.5 16.5V1.5ZM12 1.5H6V16.5H12V1.5Z" fill="white"/>
            <path d="M7.5 4H10.5V5H7.5V4Z" fill="white"/>
          </svg>
        </span>
        <span style="display: inline-block; vertical-align: middle;">Download on the App Store</span>
      </a>
    </p>`;
  }
