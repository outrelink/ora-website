...
function generateEmailTemplate(options = {}) {
  ...
  if (personalizedMessage.includes('[App Store]') || personalizedMessage.includes('[Download on App Store]')) {
    if (appId) {
      // Use Apple's official App Store badge with proper URL encoding
      const badgeUrl = `https://tools.applemediaservices.com/api/badges/download-on-the-app-store/black/en-us?size=250x83&releaseDate=2024-01-01`;
      htmlMessage += `<p style="margin: 30px 0 20px 0; text-align: center;">
        <a href="${finalAppStoreUrl}" target="_blank" rel="noopener noreferrer" style="display: inline-block; text-decoration: none;">
          <img src="${badgeUrl}" 
               ...
               style="height: 60px; width: auto; max-width: 200px; display: block; margin: 0 auto; border: none; background: transparent;" 
               border="0"
               onerror="this.style.display='none'; this.nextElementSibling.style.display='inline-block';">
          <span style="display: none; padding: 14px 28px; background: linear-gradient(135deg, ${primaryColor} 0%, #ea580c 100%); color: #ffffff; text-decoration: none; border-radius: 12px; font-weight: 600; font-size: 16px; box-shadow: 0 4px 12px rgba(249, 115, 22, 0.3);">Download on App Store</span>
        </a>
      </p>`;
    } else {
      htmlMessage += `<p style="margin: 20px 0; text-align: center;">
        <a href="${finalAppStoreUrl}" target="_blank" rel="noopener noreferrer" style="display: inline-block; padding: 14px 28px; background: linear-gradient(135deg, ${primaryColor} 0%, #ea580c 100%); color: #ffffff; text-decoration: none; border-radius: 12px; font-weight: 600; font-size: 16px; box-shadow: 0 4px 12px rgba(249, 115, 22, 0.3);">
